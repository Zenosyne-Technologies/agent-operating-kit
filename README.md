...
| **Size-routed dispatch** | Every task carries a `size:` t-shirt label, and the label decides which model tier executes it — right down to a micro profile for mechanical work. The orchestrator's model is the ceiling: after each failed round it reads the failure pattern — a converging task stays put, a stuck one climbs an effort ladder on that model, one churning, bloating or looping gets its approach rethought instead — and only at `max` does the orchestrator ask: `max` effort (recommended) or a swap to the frontier model. |
...
| **A cascading ruleset** | One always-loaded core — your `CLAUDE.md` plus the kit core it imports — holds only what applies to every turn; per-activity rules live beside it and are *referenced* in briefs, never inlined. Context stays proportional to the task. |
...
2. **Copy the payload.** `templates/marvin/agents/` → `<repo>/.marvin/agents/`; your PM tool's `templates/pm/<tracker>/tracker-config.md` and `stats-collection-brief.md` → `<repo>/.marvin/agents/`; Jira only, `templates/pm/jira/convert-milestones-brief.md` → `<repo>/.marvin/agents/`; `templates/marvin/PROJECT-INFO.md` and `templates/marvin/MEMORY.md` → `<repo>/.marvin/`; the estate seeds file by file, NOT as a directory copy — `templates/docs/index.md` (the crawl's entry point), `templates/docs/{plans,researches,refactor,future,information,release-notes}/index.md` and `templates/docs/handbooks/index.md` each → the same path under `<repo>/.docs/`, then `templates/docs/handbooks/audience-index.md` → `<repo>/.docs/handbooks/developer/index.md`, `…/user/index.md` and `…/admin/index.md` (three copies under that one name; `audience-index.md` itself is never placed in `.docs/` — an unindexed file there is unreachable by the crawl, which means it does not exist). An index the repo already has is merged — keep its rows and prose, add only the rows it lacks: the missing sub-folder rows, and the `## Root-level documents` section with its issue-log row where that section does not exist yet; `templates/marvin/CLAUDE.marvin.md` → `<repo>/.marvin/CLAUDE.marvin.md` (the kit core — refreshed on every upgrade, never edit it); `templates/CLAUDE.project.md` → `<repo>/CLAUDE.md` — an existing `CLAUDE.md` keeps every line and gains only the stub's missing project-rules items plus the bare import line `@.marvin/CLAUDE.marvin.md` as its LAST line, never in backticks or a code block (Claude Code skips imports there); a repo with only an `AGENTS.md` gets the stub with `@AGENTS.md` directly above that import; `templates/settings.json` → `<repo>/.claude/settings.json`, merging if one exists. Finally, list every `.md` under `.docs/` (outside `project-management/` and `reports/`) that no index row reaches — give each an index row where its folder is obvious, and name the rest: under these rules an unindexed document cannot be found. Add `.marvin/runs/` to `<repo>/.gitignore` — sub-agent run reports live there and are never committed.
...
**From v0.33.0, the kit's rules live in `.marvin/CLAUDE.marvin.md`, not in `CLAUDE.md`.** `CLAUDE.md` is yours: project facts, your project rules, and one bare line, `@.marvin/CLAUDE.marvin.md` — Claude Code's native import, resolved relative to `CLAUDE.md` and loaded at session start with no prompt. The kit file is kit-owned, so every upgrade refreshes it like any `.marvin/agents/` guide and no release ever again asks you to re-sync `CLAUDE.md` by hand. The first upgrade to v0.33.0 splits your existing `CLAUDE.md` once: a read-only planner (`scripts/plan-claude-md-split.sh`) classifies every line, the upgrade asks ONE confirmation question before it writes anything at all, keeps a verbatim backup under `.marvin/backups/`, removes only the kit's own lines, lets you keep or drop each customised kit rule it flags for review, and appends the import; declining writes nothing but the report. Project rules in `CLAUDE.md` refine the kit's; the kit's safety gates cannot be switched off from there. Imports nest at most four hops — `CLAUDE.md` → the kit core is one, and the kit core imports nothing. Keep `.marvin/` tracked: a gitignored kit file makes the import load nothing in every other clone.

**`AGENTS.md` is supported only through `CLAUDE.md`.** Claude Code reads `AGENTS.md` only when no `CLAUDE.md` exists, so a `CLAUDE.md` that should honour it carries an `@AGENTS.md` line (install adds it when it creates `CLAUDE.md` beside an existing `AGENTS.md`). The kit never writes `AGENTS.md` and does not support it as the sole or only-edited instruction file.

**A symlink on any path the upgrade touches refuses the whole of its step 4** — `CLAUDE.md`, `.marvin/CLAUDE.marvin.md`, `.marvin`, `.marvin/agents`, `.marvin/backups`, `.docs`, `.claude/agents` and their ancestors are all tested. The `CLAUDE.md → AGENTS.md` monorepo layout hits this, as does a `.claude/agents` symlinked into a dotfiles repo. It is a refusal to **clear**, not a bug — replace the link with a real file (for `CLAUDE.md → AGENTS.md`: a `CLAUDE.md` containing `@AGENTS.md`), or move it aside for the run: writing (or, for the persona cleanup, deleting) through a link destroys files outside the repository while `git status` stays clean, with nothing backed up to reconcile from.
...
flowchart TD
    ...
    EH --> MG{"Max gate: ask the user<br/>max effort (recommended) or frontier?"}
    MG -->|max, no answer, or unattended — recorded| EM["Orchestrator's model at max effort"]
    MG -->|frontier, explicitly chosen| EF["Frontier tier, this task only"]
...
- **A version-drift check.** It compares the plugin's own version against the project's installed `kit_version` (`.marvin/PROJECT-INFO.md` frontmatter, numerically, component by component) and, on a mismatch, prints one line: if the project is older, it points at `/marvin:upgrade-agent-os`; if the project is newer than the plugin, it says so and points at updating the plugin instead, since running the upgrade skill there would downgrade the install. An unreadable or symlinked `PROJECT-INFO.md`, or a `kit_version` that is not a plain `X.Y.Z` in the frontmatter, counts as unknown and gets its own one-line nudge to run the upgrade skill — never a raw file dump.
...
It never writes a file, never blocks the session, and always exits 0 — including when `.marvin`, `plugin.json` or `PROJECT-INFO.md` is missing or unreadable. Editing `hooks/hooks.json` in the installed plugin cache does not survive the next plugin update, so to disable it durably, either disable the whole plugin (`/plugin disable marvin`) or turn off all hooks via the `"disableAllHooks": true` setting ([hooks reference](https://code.claude.com/docs/en/hooks.md)); there is no flag that disables only this hook.
...
| `upgrade-agent-os` | Bringing an existing install up to the plugin's current version — file migrations, new cascade docs, the kit core's refresh, tracker label re-sync; the one-time `CLAUDE.md` split and relabel sweeps each gated behind one confirmation. |
...
**Context proportionality.** `CLAUDE.md` and the kit core it imports (`.marvin/CLAUDE.marvin.md`) are the only always-loaded files, and they hold only rules that apply to *every* turn. Everything else lives in the `.marvin/agents/` cascade and is loaded solely when that activity is happening — briefing, guardrails, validation, documentation, ticket filing, labeling, tracker configuration, planning research, reporting, token economics, handbooks, security, micro-tasks. Briefs cite the file; they never inline its content.
...
templates/
  CLAUDE.project.md                consumer-owned stub that seeds the project's root instructions file — facts, project rules, the bare kit-core import last
  ...
  marvin/
    CLAUDE.marvin.md               kit-owned always-loaded core rules (installed to .marvin/, imported by the project file, refreshed on upgrade)
  ...
  marvin/agents/
    ...
    escalation-ladder.md           the effort escalation ladder — high → xhigh → the ask-at-max gate (max recommended, frontier opt-in) → stop at the user
    model-profile.md               the version's declared model set — tier → concrete model → default effort, model-specific rules, validated-set statement
...
- **Each version targets a declared model set.** Templates name tiers (`orchestrator` / `heavy worker` / `small worker` / `micro` / `frontier`); `templates/marvin/agents/model-profile.md` alone maps them to concrete models, and install writes those names in — no install-time substitution. v0.32's set: Opus 5.5 as orchestrator and heavy worker, Sonnet 5 as small worker, Haiku 4.5 as micro, Fable 5.1 as the opt-in frontier — default efforts and the model-specific rules (the max gate recommends `max` over frontier) are the profile's. Other models are unsupported: the orchestrator says so once when its session model differs from the profile's.
