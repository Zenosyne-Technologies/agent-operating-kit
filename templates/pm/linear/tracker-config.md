...
- Workspace with native **Releases** → the version is a Release named `v<version>`, matching the tag; the issues in the frozen scope attach to it. GitHub's tag-is-the-release case is the shape to copy, but not its authority: a Linear Release is tracker bookkeeping, not publishing (`.marvin/agents/git-strategy.md`).
- Workspace without them → the version lives on the annotated tag plus `.docs/release-notes/v<version>.md`, and issues carry nothing. Do NOT invent a `release:*` label to fill the gap: a hand-synced label would be a second source of truth for a tag that is already canonical.
- Either way, RELEASE scope resolves from the note's `scope:` header field, never from the tracker — that field is mandatory on a release note and reads the same with or without native Releases. MILESTONE scope stays the native project milestone. Two axes, two sources.
