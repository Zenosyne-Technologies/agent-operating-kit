...
- Tag FIRST, publish second — `gh release create` against a tag that does not yet exist invents a LIGHTWEIGHT one, which the tag rule forbids: `git tag -a v<version> -m …` on the release branch's merge commit, `git push origin v<version>`, then `gh release create v<version> --verify-tag --notes-file <body>` (a GitHub Release is publishing — authority per `git-strategy.md`; `--verify-tag` refusing to invent anything).
