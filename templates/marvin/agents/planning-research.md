...
The planner splits every `size:m` task into ONE design-bearing core plus zero or more peripheral pieces:
...
`brief-gaps` BLOCKS the build until the orchestrator has resolved every finding — amended the brief, or the DoD on the tracker issue (via `marvin:ponytail`) — or recorded why not, one line per declined finding in the build brief (`Falsifier F<n> declined: <reason>`), so builder and validators see the gap was deliberate. No re-run after resolution. A falsifier with no usable verdict is off the ladder (`escalation.md`): re-run fresh once, then the orchestrator falsifies inline. Correction dispatches skip it — validator findings are their spec.
