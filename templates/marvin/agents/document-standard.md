...
**Why `.marvin/agents/` is reduced** (settled — do not re-litigate): those guides are REACHED BY NAME — from `.marvin/CLAUDE.marvin.md`'s *Rules cascade*, or from another guide that cites them, as `reporting.md` cites `stats-collection-brief.md` — never by crawling. Whoever sent you there already named the guide and said why, so `keywords`/`level`/`covers` would serve a search that never happens. What remains earns its keep across kit upgrades: `status:` + `superseded_by:` is how a stale guide announces itself, and `summary:` lets a brief cite one without loading it. The dependency runs ONE WAY: a `.marvin/` guide or the cascade may point INTO `.docs/`, but a `.docs/` document is tool-agnostic and self-contained — it never cites `.marvin/` or names the orchestrator, because an agent reaches THIS standard through the cascade, not through a pointer embedded in a doc.
