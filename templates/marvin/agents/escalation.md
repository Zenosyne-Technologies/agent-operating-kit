...
4. The chosen final rung fails twice → **STOP**. The ladder terminates; never loop back to a lower rung. Escalate to the user per `guardrails.md`'s escalation chain: the full attempt history goes on the tracker issue, and the chat message names where it is.
