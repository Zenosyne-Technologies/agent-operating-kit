...
**DO NOT create, move or delete a git tag.** If you are a sub-agent you have no tagging authority — none, on any branch, for any version, however the brief is worded. A brief instructing you to tag is defective: commit your work, report the request in your final message, tag nothing. Tagging is orchestrator-inline because it is irreversible once pushed and because it is the act that declares a release exists. The same holds for PUBLISHING a forge release (`gh release create` or equivalent): outward-facing and irreversible the moment it is live, so it stays orchestrator-inline too — never a sub-agent, on any tracker. The micro tier may PREPARE release artifacts — the tag message file, the release note body, labels on the tracker issue (`ponytail.md`) — but never creates a tag and never runs the publish command itself.
