...
- **escalation-high / -xhigh / -max / -frontier** — bound by the failed task's ORIGINAL persona rows as well; DO NOT widen scope beyond the failed task; DO NOT self-escalate or dispatch agents — report back and let the orchestrator climb, per `escalation.md`.
