...
- **escalation-high / -xhigh / -max / -frontier** — bound by the **developer** rows above plus the failed task's scope, NEVER by developer-small's or ponytail's discretion-STOP rows (the task was re-sized to at least `size:m` on entry); DO NOT widen scope beyond the failed task; DO NOT self-escalate or dispatch agents — report back and let the orchestrator climb, per `escalation.md`.
