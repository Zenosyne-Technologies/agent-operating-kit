...
- Syntax: `dimension:value`, kebab-case values. Exceptions for continuity: severity is bare (`sev1-critical`..`sev4-low`); virtual milestones are `milestone:<slug>` and virtual releases `release:v<semver>` (only where `tracker-config.md` prescribes them).
...
| `release:<version>` | issues in a frozen release scope, ONLY where `tracker-config.md` prescribes it | `release:v<major>.<minor>.<patch>` — mirrors that version's annotated tag, which stays canonical (`git-strategy.md`) |
...
| Version | Change |
|---|---|
| 1.3.0 | Adds `release:<version>` — a virtual release scope, prescribed ONLY where `tracker-config.md` says the tracker has no native release object (Jira today; GitHub, Linear and Local map natively and must NOT use it). Mirrors the annotated tag, which stays canonical. |
