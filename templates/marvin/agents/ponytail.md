...
- **Restatement grep sweeps** (the split-responsibility check) — after a rule changes, grep every restatement of the named changed rule across the paths given and report each hit as `file:line`; it reports, it never edits the prose.
- **Pre-flight gate runs** — run the repo's named gate scripts and report PASS/FAIL per gate, before a validator is dispatched, so a mechanical failure bounces to the builder without spending a heavy validator; pre-flight never replaces a validator — a validator still runs after PASS.
