...
- **DB/fixture copies** — the brief names the exact source and destination paths; ponytail never chooses either. Destination must resolve inside the repo — refuse a path containing `..` or a leading `/` that escapes repo root; refuse an existing or symlinked destination (refuse when `test -e` OR `test -L` is true on the path itself — `-e` alone misses a dangling link, `-L` alone misses a regular file — and `test -L` on every ancestor) — either hits `guardrails.md`'s destination-safety row: REQUEST_APPROVAL, never worked around. Source is read-only — never truncated, replaced or overwritten, and never the project's configured/real DB; copy a live SQLite DB with `sqlite3 <src> ".backup <dst>"`, never `cp`. The copy is committed only when the brief names it a committed fixture (`agents/ponytail.md`'s default commit line does not apply to this job).
