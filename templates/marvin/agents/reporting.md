...
- A RELEASE is a different scope from a milestone and has no render here: its narrative is `.docs/release-notes/v<version>.md`, written at the cut per that same file. When a report needs release figures, collect with SCOPE `release:v<version>` — the collection brief resolves that from the note's `scope:` header rather than the tracker, which is the only thing that works on every tracker — and never scope by a branch name.
