...
6. **Branch + autocommit**: the milestone branch to work on (`milestone/<KEY>-<slug>`); the agent commits its own scoped work itself (atomic commits, selective add), messages starting with the tracker issue key the brief names (`<KEY>: …`) before its final message — work is never left uncommitted, and the agent never asks permission to commit.
