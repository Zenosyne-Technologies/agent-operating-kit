...
7. **Branch + autocommit**: the exact branch to work on — for a work item that is its own `feature/<KEY>-<slug>`, named in the brief, never left to the agent to derive or create; the model behind it, and what an agent may NOT do (create, move or delete a tag — no sub-agent has tagging authority), are `.marvin/agents/git-strategy.md`'s. The agent commits its own scoped work itself (atomic commits, selective add), messages starting with the tracker issue key the brief names (`<KEY>: …`) before its final message — work is never left uncommitted, and the agent never asks permission to commit.
