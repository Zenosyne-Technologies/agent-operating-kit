...
- **{{MICRO_MODEL}} subagent** ("ponytail"): `size:xs` mechanical zero-discretion micro-tasks (`marvin:ponytail`) → `.marvin/agents/ponytail.md` — including EVERY tracker call and mechanical release step whose content the orchestrator has already decided, never done inline: tracker tools return multi-KB payloads, and they belong in the micro tier's context, not the orchestrator's.
