...
Escalate EFFORT, not model: a task that fails two attempts (two build/correction dispatches, each ending not-done or validator-FAILed) at any tier climbs to the orchestrator's model one effort level higher (`marvin:escalation-high` → `marvin:escalation-xhigh`), and before `max` you ASK the user whether to swap to {{FRONTIER_MODEL}} instead and wait — only an unattended session goes straight to `max`, recording that the gate auto-passed → `.marvin/agents/escalation.md`; de-escalate when work turns mechanical.
