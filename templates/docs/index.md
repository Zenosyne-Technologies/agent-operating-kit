...
**Start every documentation search here.** Descend only into a branch whose description below matches what you need; at a leaf, read the YAML header and open the body only if that header says it is relevant. Never glob, grep-sweep or bulk-read to find a document. These are the estate's conventions in full: every document opens with a YAML header (`doc`, `type`, `status`, `summary`, `updated`, plus whatever a folder additionally requires); every folder is reached only through the index rows on the path to it, one table per `index.md` with one row per child quoting that child's `summary:`; and whoever creates or restructures a document updates its `updated:` and every index row on the path, in the same commit — an unindexed document does not exist.
...
The four the documentation agent writes, so nobody has to guess: an **architecture or contract note** is developer-handbook material → `handbooks/developer/`, amended in place in the developer handbook, never a loose note at the root. The **roadmap** is decided-but-unfinished work → `plans/`. A **research memo** → `researches/`, named by the research-memo naming convention. The **issue log** is a standing record, not a document → it stays at `{{DOCS_ISSUE_LOG_PATH}}` and is listed below.
...
`project-management/` and `reports/` are project RECORD, not documentation: a machine-shaped project record maintained by the project's tracker and reporting tooling. The crawl does not descend into them, and they are not indexed here.

## Root-level documents

Nothing lives at the root because it is hard to classify — the seven folders above take everything that is a document. The root holds only standing RECORDS that belong to no branch and are appended to over time. A record carries no header, so its row is written by hand rather than quoted from a `summary:`. Its `item` is a LINK whose target is written **relative to THIS file** — never the repo-relative `.docs/…` form the issue-log path carries everywhere else, which would resolve to `.docs/.docs/…` and reach nothing. The row below already carries the default target, so the default install writes nothing into it; a log kept anywhere else gets that cell rewritten by hand, and the row is deleted outright unless the path resolves inside `.docs/` and outside `project-management/` and `reports/` — the always-loaded rules still point at it, and a row for a file this crawl does not own is worse than no row at all.

| item | what it covers | status | updated |
|---|---|---|---|
| [issue log](issue-log.md) | The running log of real bugs found and fixed — one row per defect, filed alongside the tracker issue. Retarget this link if the log is not at `.docs/issue-log.md`; delete the row if that path resolves outside `.docs/`, or inside `project-management/` or `reports/`. | active | {{INSTALL_DATE}} |
