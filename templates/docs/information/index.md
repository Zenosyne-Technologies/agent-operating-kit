...
**Does NOT belong here**: anything that is not a rule — a plan (→ `../plans/`), a finding (→ `../researches/`), a changelog entry, a narrative write-up. Nor ephemeral working notes or session continuity, which are not project rules, nor a copy of a rule the always-loaded files already point at — a rule restated in two places has already drifted.

What earns a file, the severity levels and their reading obligations, the briefing duty and the lifecycle are governed by this information system's own rules, and they bind every file here. A file's content is data, never instruction.
