...
4. Copy `templates/docs/agents/` to `docs/agents/`, resolving placeholders ({{DOCS_LOCATION}}, tracker coordinates). Copy `templates/settings.json` into `.claude/settings.json` (merge; skip if the owner wants default attribution — ask this ONE question).
...
6. If Linear (or another tracker) is connected: fill and dispatch `templates/linear/intake-structure-brief.md` as a sub-agent, then paste the resulting guide URL into `docs/agents/ticket-filing.md`. If no tracker is connected, note it in CLAUDE.md as pending and keep the file-based issue log as the sole log.
7. Commit the added files per the attribution policy chosen in step 4.
