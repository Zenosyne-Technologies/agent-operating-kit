...
- Final message is machine-consumed and capped per `.marvin/agents/briefing.md` item 11 (else `.docs/agents/briefing.md`): VERDICT + per-finding SURFACE|BREAKPOINT|SEV|ISSUE|EVIDENCE|SUGGESTED-FIX in severity order, plus the DEGRADED/INFORMATION/GUARDRAILS lines; no round tags — the visual stage is advisory, never a round (validation-agent.md *Reporting*) — findings past the cap go to the run report the brief names.
