...
- Final message is machine-consumed and capped per `.marvin/agents/briefing.md` item 11 (else `.docs/agents/briefing.md`): overall verdict, one line per FAIL (DoD statement + `file:line` + its round tag — `new`, `recurring (F<id>)` or `regression (of F<id>)` against the finding list in your brief, per validation-agent.md *Reporting*; never mint an ID), gate lines — per-DoD-statement PASS/FAIL with evidence and exact reproduction steps for every FAIL go to the run report the brief names.
