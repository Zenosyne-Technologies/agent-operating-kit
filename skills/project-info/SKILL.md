...
2. Write `.marvin/PROJECT-INFO.md` from the template with every placeholder resolved — every YAML frontmatter key filled (`kit_version` from the plugin's `.claude-plugin/plugin.json` when installed via plugin, else the kit repo's; `label_syntax_version` from the installed `label-syntax.md`'s H1; `screens_guide` → the UI screen catalog pointer, default `.docs/handbooks/developer/screens/index.md`; `visual_validation` → `per-task` | `milestone` | `off`, default `per-task`); body stays facts-only prose, no operating rules.
