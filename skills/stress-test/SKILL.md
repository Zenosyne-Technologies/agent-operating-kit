...
1. **Map the scale dimensions**: have reader sub-agents (micro/small tier) walk the project (and `.docs/handbooks/developer/` if present) and return per-axis excerpts (`file:line`) for every axis that grows in production — collection sizes, repeaters/nested forms, concurrent users/sessions, payload and file sizes, query result widths, background queue depth, integration fan-out. From those excerpts, list each axis with the scale demos used, the scale production will see, and the scale where you'd EXPECT trouble.
