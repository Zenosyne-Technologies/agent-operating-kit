...
2. **Diff against current templates**: file moves, cascade files to add, cascade lines to add/update in the consumer CLAUDE.md, label-registry delta (installed `label_syntax_version` → current registry H1), tracker-config/brief updates for the installed `pm_tool`, the `telemetry` frontmatter key.
