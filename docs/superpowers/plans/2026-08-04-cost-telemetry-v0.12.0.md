...
**Architecture:** Two repos. KIT (this repo, branch `claude/cost-telemetry-v0.12.0`): markdown payload + skills + gate. TELEMETRY (`~/Dev/agent-token-telemetry`, branch `claude/v0.2.0` off main): Python capture + SQLite schema + commands. The spec is the requirements source; this plan pins exact mechanics.
...
**Files (in ~/Dev/agent-token-telemetry, branch `claude/v0.2.0` off main):** Modify `scripts/capture.py`, `tests/test_capture.py`.
