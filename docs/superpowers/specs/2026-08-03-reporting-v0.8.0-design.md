...
Resolved post-validation in this release: the snapshot schema gains `defects_by_area` (16 top-level keys, still stats_schema 1 — pre-release change) and the digest/close-out wording now names only collected fields.
